...
# function to detect faces n landmarks
def detect(img):
    #using 2.5g parameters
    detector = SCRFD(model_file='./scrfd_weights/scrfd_10g_kps.onnx')
    detector.prepare(-1)
    
    img = cv2.imread(img)

    bboxes, kpss = detector.detect(img, 0.4)
    # print(bboxes,kpss)
    # for _ in range(1):
    ...
    for i in range(bboxes.shape[0]):
        ...
        # found a cropped face in the image, then skip
        if(x1<0 or y1 <0 or x2 <0 or y2 < 0 or y2 > img.shape[0] or x2 >img.shape[1]):
            continue
...
if __name__ == '__main__':
    ...
    detector = SCRFD(model_file='./scrfd_weights/scrfd_2.5g_kps.onnx')
    detector.prepare(-1)
    img_paths = ['../test.jpg']
